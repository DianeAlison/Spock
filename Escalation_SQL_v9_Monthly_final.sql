...
SET			@start_date =		'01/01/2018'    ---JFM ---Change this to start - 12 or 24 months...need to validate
SET			@end_date	=		'12/31/2020'-- Change the end_date to the last day of the previous month.
...
SELECT		t.car_id,
	...
	case	when (t.ICR_action = 1 or t.ICR_status = 1) then 1 else 0 end as 'ICR_touch',
	case	when (t.PCR_action = 1 or t.PCR_status = 1) then 1 else 0 end as 'PCR_touch',
		    t.ICR_bypass,
			t.ICR_scored,
			t.PCR_scored,
			t.sent_to_AI,
