...
--Aetna by market version
--Currently used for Aetna only

--select top 50 * from ASDReportDB.dbo.Aetna_Phys_Med_Determs
...
--select *, DATEADD(dd,1,end_date) from #rpt_parms
---------------------------------------------------
--Aetna - By Market Report
---------------------------------------------------
select	'**** Determ Activity Summary ****'

IF OBJECT_ID('tempdb..#summ1') IS NOT NULL drop table #summ1

--Get summary counts by market, by provider type
select	Market, --Urgency,
		Provider_Type,
...
where	car_name = 'Aetna'
		and determination_date between @start_date and dateadd(dd,1,@end_date)
group by Market, Provider_Type

UNION ALL 

--Get summary counts by market only, to get the subtotal per market
select	Market = market + ' - TOTAL', --Urgency,
		Provider_Type = '',
		Total_Episodes = count(distinct(tracking_number)),
		Total_Determs = count(auth_id),
		Certified = sum(case when UM_Determ_Outcome = 'Certified' then 1 else 0 end),
