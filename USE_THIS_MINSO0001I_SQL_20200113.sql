...
01/13/20
add pm queues sg2 and sgm --MG

*/

IF OBJECT_ID('AdHoc.dbo.pcrqueue2j') IS NOT NULL 
drop table AdHoc.dbo.pcrqueue2j;

IF OBJECT_ID('AdHoc.dbo.pcrqueue3j') IS NOT NULL 
drop table AdHoc.dbo.pcrqueue3j;

IF OBJECT_ID('AdHoc.dbo.pcrqueue4j') IS NOT NULL 
drop table AdHoc.dbo.pcrqueue4j;

IF OBJECT_ID('AdHoc.dbo.pcrqueue5j') IS NOT NULL 
drop table AdHoc.dbo.pcrqueue5j;

IF OBJECT_ID('AdHoc.dbo.pcrqueue6j') IS NOT NULL 
drop table AdHoc.dbo.pcrqueue6j;
...
----set @last_night			=    'Jan 10 2019 10:00PM'   ---  THIS IS THE TWO DAYS BEFORE THE RUN delivered  ---EX 3.22.16 for the report DELIVERED 3/24
----set @last_night_end  =   'Jan 11 2019 10:00PM'    ----  THIS IS THE DAY BEFORE THE RUN delivered ---EX 3.23.16 for the report DELIVERED 3/24

set @last_night			=  '02/16/2020 10:00pm'
set @last_night_end =  '02/17/2020 10:00pm'  
...
create table AdHoc.dbo.pcrqueue2j
...
SELECT db_name, client_car_name
...
select 	@ls_sqlstring = 'insert into AdHoc.dbo.pcrqueue2j (healthplan, auth_id, date_in, date_out, queue_code, outbound_queue_code) '+'select ''' + @car_name + ''',	aqh.auth_id, date_in = aqh.date_queued,
...
where aqh.queue_code in (''mdr'' , ''um'' , ''spr'' , ''iiq'' , ''pm2'' , ''rr2'', ''sg2'', ''sgm'') 
           and (aqh2.date_queued >= ''' + @last_night + '''  or aqh2.date_queued is null)

union

select ''' + @car_name + ''',	aqh.auth_id, date_in = aqh.date_queued, date_out = aqh2.date_queued,
	aqh.queue_code, aqh2.queue_code     	
from  ' + @db_name + '..auth_queue_history_arch aqh WITH(NOLOCK) 
          left join  ' + @db_name + '..auth_queue_history_arch aqh2 WITH(NOLOCK)  on (aqh.auth_id = aqh2.auth_id
						and aqh2.date_queued = (select min(aqh3.date_queued) 
						from  ' + @db_name + '..auth_queue_history_arch aqh3 WITH(NOLOCK) 
						where aqh3.auth_id = aqh.auth_id
							and aqh3.date_queued > aqh.date_queued))
where aqh.queue_code  in (''mdr'' , ''um'' ,  ''spr'' , ''iiq'' , ''pm2'', ''rr2'' , ''sg2'', ''sgm'') 
           and aqh2.date_queued >= ''' + @last_night + ''' '
...
into	adhoc.dbo.pcrqueue3j      ----------- select * from adhoc..pcrqueue3j     where  outbound_queue_code <> 'fhs'     
from	adhoc.dbo.pcrqueue2j a
		join niacore..informa_queues b WITH(NOLOCK)  on (a.queue_code = b.queue_code)
...
from	adhoc.dbo.pcrqueue3j 

group by sort_key,queue_desc,last_night
order by sort_key
...
into	adhoc.dbo.pcrqueue4j    --- select * from adhoc.dbo.pcrqueue4j 
from	adhoc.dbo.pcrqueue3j 
...
into	adhoc.dbo.pcrqueue5j
from	adhoc.dbo.pcrqueue4j p4
...
into	adhoc.dbo.pcrqueue6j    
from	adhoc.dbo.pcrqueue5j p5
...
order by  
	case Queue_Desc 
		when 'MD Review Queue' then 0
		when 'UM Gatekeeper Review Queue' then 1
		when 'Specialist Review Queue' then 2
		when 'Insufficient Information Queue' then 3
		when 'Pain Injections MD Review Queue' then 4
		when 'Retro MD Review Queue' then 5
		when 'MSK Spine Surgery MD Review Queue' then 6
		when 'MSK Non-Spine Surgery MD Review Queue' then 7
	end  ,
Healthplan
