--Server:			nile-r1
--dB:					ASDReportdb | niacombine | nicarore | 

set nocount on
---------------------------------------------------------------------------------------------STEP 1: Pull auth status changes in the time period
declare
 @start_date datetime
,@end_date datetime
...
and aschg.user_name in (select log_id from niacore..is_users iu where iu.type = 'md')
...
and iu.log_id in (select log_id from niacore..is_users iu2 where iu2.type = 'md')
order by
date_entered
...
from
 adhoc..temptest_929
where 
 status_desc not in (Select description from niacore.dbo.auth_action_codes where auth_action_code in (874,1062,1128))
order by
 user_name
,date_changed
